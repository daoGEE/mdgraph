    ...
    switch (name) {
      case "mdgraph_search":
        return this.withRepository(projectRoot, (repository) => {
          ...
          return withFreshnessNotice(textResult(formatSearch(results), { projectRoot, query, mode, results }), freshness);
        });
      ...
      case "mdgraph_node":
        return this.withRepository(projectRoot, (repository) => {
          ...
          if (resolution.status === "not_found") {
            return withFreshnessNotice(textResult(`Node not found: ${query}`, { projectRoot, query, error: resolution.error, node: null }), freshness);
          }
      ...
      case "mdgraph_trace":
        return this.withRepository(projectRoot, (repository) => {
          ...
          const freshness = safeFreshness(projectRoot, repository);
          return withFreshnessNotice(textResult(formatTrace(trace), { projectRoot, trace }), freshness);
        });
      default:
...
function safeFreshness(projectRoot: string, repository: GraphRepository, config = tryLoadConfig(projectRoot)): StatusFreshness {
  if (!config) {
    return {
      ...
      recommendation: "freshness check failed while loading .mdgraph/config.json; run `mdgraph status --freshness --json` or `mdgraph index` before relying on results"
    };
  }
  return computeStatusFreshness(projectRoot, config, repository);
}

function tryLoadConfig(projectRoot: string): MDGraphConfig | undefined {
  try {
...
function withFreshnessNotice(result: McpToolResult, freshness: StatusFreshness): McpToolResult {
  const structuredContent = appendFreshnessStructuredContent(result.structuredContent, freshness);
  if (freshness.state === "fresh") {
    return structuredContent === result.structuredContent ? result : { ...result, structuredContent };
  }

  const relevantIssues = freshness.state === "stale"
    ? relevantFreshnessIssues(freshness.issues, structuredContent)
    : [];

  const [first, ...rest] = result.content;
  const warning = formatFreshnessNotice(freshness, relevantIssues);
  if (!first) {
    return {
      ...result,
...
function appendFreshnessStructuredContent(structuredContent: unknown, freshness: StatusFreshness): unknown {
  if (!structuredContent || typeof structuredContent !== "object" || Array.isArray(structuredContent)) {
    return structuredContent;
  }
  return { ...structuredContent, freshness };
}

function formatFreshnessNotice(
  freshness: StatusFreshness,
  relevantIssues: Array<{ path: string; reason: "added" | "deleted" | "modified" }>
): string {
  if (freshness.state === "unknown") {
    return [
      `⚠️ MDGraph freshness warning — ${freshness.recommendation}.`,
      "Run `mdgraph status --freshness --json` or `mdgraph index` before relying on these results."
    ].join(" ").replace(/\.\./g, ".");
  }

  const relevantSummary = relevantIssues.length
    ? ` Returned paths touched after indexing: ${relevantIssues.slice(0, 5).map((issue) => `${issue.path}:${issue.reason}`).join(", ")}.`
    : "";
  const globalSummary = !relevantIssues.length && freshness.issues?.length
    ? ` Markdown changed elsewhere in the workspace: ${freshness.issues.slice(0, 5).map((issue) => `${issue.path}:${issue.reason}`).join(", ")}.`
    : "";
  const guidance = relevantIssues.length
    ? "Read those docs directly for live prose, or run `mdgraph index` / prefer default `mdgraph serve --mcp`."
    : "Run `mdgraph index` or prefer default `mdgraph serve --mcp` for live sessions before relying on relationship/ranking details.";
  return `⚠️ MDGraph freshness warning — ${freshness.recommendation}.${relevantSummary}${globalSummary} ${guidance}`.replace(/\.\./g, ".");
}

function relevantFreshnessIssues(
  issues: Array<{ path: string; reason: "added" | "deleted" | "modified" }> | undefined,
  structuredContent: unknown
): Array<{ path: string; reason: "added" | "deleted" | "modified" }> {
  if (!issues?.length) {
    return [];
  }
  const paths = referencedPaths(structuredContent);
  if (!paths.length) {
    return [];
  }
  const pathSet = new Set(paths);
  return issues.filter((issue) => pathSet.has(issue.path));
}

function referencedPaths(structuredContent: unknown): string[] {
  if (!structuredContent || typeof structuredContent !== "object" || Array.isArray(structuredContent)) {
    return [];
  }
  ...
  if (Array.isArray(record.results)) {
    return [...new Set(record.results.flatMap((result) => {
      const documentPath = typeof (result as { document?: { path?: unknown } }).document?.path === "string"
        ? (result as { document: { path: string } }).document.path
        : undefined;
      return documentPath ? [documentPath] : [];
    }))];
  }

  const contextItems = (record.context as { items?: Array<{ path?: unknown }> } | undefined)?.items;
  if (Array.isArray(contextItems)) {
    return [...new Set(contextItems.flatMap((item) => typeof item.path === "string" ? [item.path] : []))];
  }

  const nodePath = (record.node as { data?: { path?: unknown } } | undefined)?.data?.path;
