...
exportCommand
  ...
  .description("Export a WikiLink-based Markdown graph index")
  .option("--path <path>", "Project root. Defaults to the current working directory")
  .action((options: { path?: string }) => {
    ...
    try {
      const graph = buildGraphJsonExport(projectRoot, repository);
      console.log(formatWikiLinkMarkdownIndex(graph));
    } finally {
      closeRepository(repository);
    }
  });
...
exportCommand
  ...
  .option("--json", "Print JSON output")
  .option("--provider <provider>", "Bridge provider", "json")
  ...
  .action((options: { json?: boolean; provider: string; artifact?: string; path?: string }) => {
    if (options.provider !== "json") {
      throw new Error(`Unsupported source bridge provider: ${options.provider}. MDGraph 1.0 supports the read-only json provider.`);
    }
    const repository = openRepository(projectRootFromOption(options.path));
    try {
      const report = buildSourceBridgeReport(repository, { artifact: options.artifact });
...
program
  ...
  .option("--watch", "Keep the Markdown graph fresh while serving MCP requests (default)")
...
function buildUsageGuide(projectRoot: string): UsageGuide {
  ...
  return {
    ...
    workflows: [
      ...
      {
        name: "Agent MCP",
        purpose: "Serve MDGraph to an MCP client with integrated watch-based freshness by default.",
        commands: [
          `mdgraph serve --mcp --path ${project}`,
          `mdgraph serve --mcp --debounce 250 --path ${project}`,
          `mdgraph serve --mcp --no-watch --path ${project}`
        ],
        notes: ["MCP serve starts one index and keeps Markdown changes fresh by default; use `--no-watch` for read-only sessions."]
      }
    ]
  };
}
