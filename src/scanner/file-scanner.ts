...
import { BUILT_IN_DOC_EXCLUDES } from "../config/load-config.js";
...
export async function resolveIgnorePatterns(projectRoot: string, config: MDGraphConfig): Promise<string[]> {
  return uniquePatterns([
    ...BUILT_IN_DOC_EXCLUDES,
    ...config.docs.exclude,
    ...(config.index.followGitignore ? await readGitignorePatterns(projectRoot) : [])
  ]);
}
...
function resolveIgnorePatternsSync(projectRoot: string, config: MDGraphConfig): string[] {
  return uniquePatterns([
    ...BUILT_IN_DOC_EXCLUDES,
    ...config.docs.exclude,
    ...(config.index.followGitignore ? readGitignorePatternsSync(projectRoot) : [])
  ]);
}
