...
Install the CLI, then replace the project path before use:

- `npm install -g @daogee/mdgraph`
- `/absolute/path/to/project`

If the host does not inherit your shell `PATH`, replace `mdgraph` in the examples with the absolute path reported by `which mdgraph` (macOS/Linux) or `where mdgraph` (Windows).
