...
- [`mdgraph-agent-instructions.md`](mdgraph-agent-instructions.md): shared instruction template for project rules or agent guidance.
- [`mcp-config.example.json`](mcp-config.example.json): stdio MCP server configuration template.
- [`host-examples.md`](host-examples.md): host-specific setup notes for Claude Code, Cursor, Copilot Chat, Codex CLI, and generic MCP clients.
...
Install the CLI with `npm install -g @daogee/mdgraph`, then replace `/absolute/path/to/project` in `mcp-config.example.json`. If the host cannot resolve `mdgraph`, use the absolute executable path reported by `which mdgraph` (macOS/Linux) or `where mdgraph` (Windows).

Use `mdgraph report --benchmark <file>` for structured comparison records, and keep full transcripts or private evaluation corpora outside the public repository.
