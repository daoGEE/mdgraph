...
try {
  ...
  const packageRoot = path.join(installDir, "node_modules", ...packageJson.name.split("/"));
  ...
  assertPackedFile(packedFiles, "dist/index.d.ts");
  ...
  fs.writeFileSync(importSmokePath, [
    `const mod = await import(${JSON.stringify(packageJson.name)});`,
    "if (!mod.indexProject) {",
  ...
  const typeConsumerPath = path.join(installDir, "type-consumer.ts");
  fs.writeFileSync(typeConsumerPath, [
    `import { DEFAULT_CONFIG, type MDGraphConfig } from ${JSON.stringify(packageJson.name)};`,
    "const config: MDGraphConfig = DEFAULT_CONFIG;",
    "void config;",
    ""
  ].join("\n"), "utf8");
