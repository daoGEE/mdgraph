...
try {
  ...
  runDoctorGateSmoke();
  runDoctorScopeSmoke();
  runContractRegistrySmoke();
  runFreshnessDiagnosticsSmoke();
} finally {
...
function runCleanProjectSmoke() {
  ...
  assert(markdownIndex.stdout.includes("[[docs/auth-v2-design.md]]"), "export markdown-index should emit WikiLink document links");
  ...
  const bridge = runCliJson(root, ["export", "source-bridge", "--json", "--artifact", sourceBridgeArtifact]);
  assertEqual(bridge.status, "ready", "source bridge should read a fixture JSON artifact");
...
function runContractRegistrySmoke() {
  const help = runCli(repoRoot, ["help"]);
  const registered = new Set();
  let inCommands = false;
  for (const line of help.stdout.split(/\r?\n/)) {
    if (line.trim() === "Commands:") {
      inCommands = true;
      continue;
    }
    if (!inCommands) continue;
    if (line.trim() === "" || /^[A-Z][a-z]+:/.test(line)) continue;
  ...
  for (const command of required) {
    assert(registered.has(command), `mdgraph help should expose "${command}" so the 1.0 CLI surface stays intact`);
  }

  const root = makeTempRoot("mdgraph-cli-registry-");
  writeCleanDocs(root);
  runCli(root, ["init", "--docs", "docs/**/*.md"]);
  const invalidGraphJson = path.join(root, "invalid-graph.json");
  fs.writeFileSync(invalidGraphJson, JSON.stringify({ format: "wrong-format", formatVersion: 1, nodes: [], edges: [], counts: {}, exportedCounts: {} }), "utf8");
  const verifyResult = runCli(root, ["import", "graphjson", invalidGraphJson, "--verify", "--json"], { expectedExitCode: 1 });
  const verifyReport = JSON.parse(verifyResult.stdout);
  assertEqual(verifyReport.valid, false, "invalid GraphJSON verify should report valid: false");
  assert(verifyReport.errors.some((error) => error.code), "invalid GraphJSON verify should expose a stable error code");

  const unsupportedProvider = runCli(root, ["export", "source-bridge", "--provider", "unsupported", "--json"], { expectedExitCode: 1 });
  assert(unsupportedProvider.stderr.includes("Unsupported source bridge provider: unsupported"), "unsupported source-bridge providers should fail with a stable error message");

  const unsupportedBridge = runCliJson(root, ["export", "source-bridge", "--json"]);
  assertEqual(unsupportedBridge.format, "mdgraph-source-bridge", "source-bridge should expose the mdgraph-source-bridge format");
  assertEqual(unsupportedBridge.status, "unsupported", "source-bridge without --artifact should be unsupported");
}

function runFreshnessDiagnosticsSmoke() {
  const root = makeTempRoot("mdgraph-cli-freshness-");
  writeCleanDocs(root);
  runCli(root, ["init", "--docs", "docs/**/*.md"]);
  const fresh = runCliJson(root, ["status", "--freshness", "--json"]);
  assertEqual(fresh.freshness.state, "fresh", "status --freshness should report fresh after init");
  assert(typeof fresh.freshness.lastIndexedAt === "string", "status --freshness should include lastIndexedAt");

  const combined = runCliJson(root, ["status", "--storage", "--freshness", "--json"]);
  assert(combined.storage?.database?.pageSize > 0, "status --storage --freshness should still include database page size");
  assertEqual(combined.freshness.state, "fresh", "status --storage --freshness should report fresh");
  ...
  const stale = runCliJson(root, ["status", "--freshness", "--json"]);
  assertEqual(stale.freshness.state, "stale", "status --freshness should report stale after a Markdown change");
  assert(stale.freshness.issues.some((issue) => issue.path === "docs/auth-v2-design.md" && issue.reason === "modified"), "status --freshness should list the modified Markdown path");

  const plainStatus = runCliJson(root, ["status", "--json"]);
  assert(plainStatus.freshness === undefined, "status --json should not include freshness diagnostics");
}
