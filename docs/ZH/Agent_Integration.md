...
先安装公开 CLI 包，所有支持 MCP 的 client 再使用相同的 stdio command：
...
如果宿主没有继承终端的 `PATH`，请用 `which mdgraph`（macOS/Linux）或 `where mdgraph`（Windows）返回的绝对路径替换 `mdgraph`。开发场景仍可使用源码 checkout 构建出的 `dist/bin/mdgraph.js`。
...
- 结构化 with/without-MDGraph run-record delta 使用 `mdgraph report --benchmark <file>`；完整 transcript 和私有评估语料应保留在公开仓库之外。
