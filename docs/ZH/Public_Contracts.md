...
| 实验性 Wiki 工作流 | experimental | `src/wiki/wiki-plan.ts`、`src/wiki/wiki-status.ts`、`src/bin/mdgraph.ts` | `wiki plan/brief/status/verify` 增加确定性规划、有界证据 brief、影响状态和只读验证；不新增 MCP 工具，也不生成或覆盖正文。WikiPlan/WikiPageBrief formatVersion 1、页面状态名、结构化 verification issue 字段以及 invalid verification 的非零退出均为 experimental。 |
...
| 顶层 CLI JSON output shapes | stable | `docs/ZH/Output_Contracts.md` | Output Contracts 中记录的必需字段稳定；命令内嵌 graph record 遵循 `src/types.ts`，除非某一 section（例如实验性结构化 `query`、`relationships derive` 或 `wiki`）另有明确标记。 |
...
| 动态 Knowledge Card 投影 | experimental / stable-additive | `src/query/knowledge-card.ts`、`src/query/context-builder.ts`、`src/bin/mdgraph.ts`、`src/mcp/tools.ts` | 找到 document、section、entity 或 source-ref 时，`node` 结果可增量返回确定性、非持久化的 `card`；已选 context item 只会使用原始内容装入后的剩余预算增加 `cardSummary`，且字符计入 `usedChars`。既有 node 字段、ranking、trace path、SQLite counts 和五工具 MCP surface 均不改变。 |
