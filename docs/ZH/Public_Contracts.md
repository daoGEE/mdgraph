...
本文记录当前 `1.0.0` public surface 的公开契约边界。它补充 [Output_Contracts.md](Output_Contracts.md)、[Architecture.md](Architecture.md) 和 [Release_Checklist.md](Release_Checklist.md)。完整的逐命令、逐格式、逐配置、逐 schema 清单见 [1.0 契约冻结附录](Public_Contracts_1.0.md)。
...
| `serve --mcp` 默认 watch 与 `--no-watch` 退路 | stable-additive | `src/bin/mdgraph.ts` | `serve --mcp` 默认保持 Markdown graph fresh，并暴露 `--no-watch` 作为只读退路；`--watch`、`--semantic` 和 `--debounce <ms>` 仍是集成 watch-based freshness 的 additive flag。 |
...
以 `1.0.0` 发布基线为准，应继续保留稳定的 1.0 surfaces，并把 0.9 additive surfaces（`usage`、项目级 `--path`、`status --freshness`、MCP freshness metadata、context recovery fields 和默认 fresh MCP serving）与 0.7-0.8 契约一起验证。
