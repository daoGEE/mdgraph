...
索引在 SQLite 元数据中记录提取配置和提交版本。提取设置变化会重建相关图谱，纯查询设置不触发重建。增量更新根据全部已解析文档协调确定性关系，因此未修改文档的别名歧义也会更新。内容与 provider/model/dimensions 匹配的向量会复用。显式全量重建清除派生关系；提取配置引发的自动重建在向量配置不变时保留未受影响的派生关系。

并发提交发现索引版本变化时重新扫描一次，持续冲突会明确报错，可稍后重试。解析失败保留该文档原有图谱，报告跳过文件，并将未完成的配置重建留待重试。修复对应 Markdown 并重新索引后再依赖其内容。

Watch 会重载配置和 `.gitignore` 规则，包括 MDX 开关。新监听器就绪后再关闭旧监听器。配置暂时无效时保留原监听并报告 degraded；修复配置后自动重试恢复。

## Node.js 构建要求

运行时需要具备启用 FTS5 的 `node:sqlite`，仅满足 22.5.0 版本号不够。Node 22.23.2 和 26.5.0 已通过本地完整测试。实测 22.5.0 和 22.13.0 标准构建缺少 FTS5，单独增加实验 SQLite 开关无法补足。MDGraph 在应用持久化 schema 之前检查实际能力，缺少时报告 `sqlite_fts5_unavailable` 并提示升级。通过扩展提供 FTS5 的构建仍可使用。

早期运行时 CI 只验证 SQLite 兼容行为与缺失能力的恢复提示，不代表标准 Node 22.5.0 可运行全部产品功能。Windows 由 CI 验证，跨平台发布前应检查对应提交的运行结果。
