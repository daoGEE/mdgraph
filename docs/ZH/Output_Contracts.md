...
`mdgraph export markdown-index` 会输出基于相同图事实的 WikiLink Markdown 视图。

## `export source-bridge --json`

`mdgraph export source-bridge --provider json --artifact source-bridge.json --json` 返回只读 source bridge report：
...
Bridge 只读取显式传入的 MDGraph JSON artifact；该 artifact 使用 `files` 数组保存项目相对路径和可选 symbol 摘要。它会在匹配前拒绝过大或过深嵌套的 JSON，不创建 graph edges，也不影响 indexing、search、context 或 MCP 工具。
