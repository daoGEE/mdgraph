...
`mdgraph export markdown-index` prints a WikiLink-based Markdown view over the same graph facts.

## `export source-bridge --json`

`mdgraph export source-bridge --provider json --artifact source-bridge.json --json` returns a read-only source bridge report:
...
The bridge reads only an explicit MDGraph JSON artifact with a `files` array containing project-relative paths and optional symbol summaries. It rejects oversized or deeply nested JSON before matching, does not create graph edges, and does not affect indexing, search, context, or MCP tools.
