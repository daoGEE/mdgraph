...
| Experimental Wiki workflow | experimental | `src/wiki/wiki-plan.ts`, `src/wiki/wiki-status.ts`, `src/bin/mdgraph.ts` | `wiki plan/brief/status/verify` adds deterministic planning, bounded evidence briefs, impact status, and read-only verification without adding MCP tools or generating/overwriting prose. WikiPlan and WikiPageBrief formatVersion 1, page-state names, structured verification issue fields, and non-zero invalid verification are experimental. |
...
| Top-level CLI JSON output shapes | stable | `docs/EN/Output_Contracts.md` | Required fields documented in Output Contracts are stable; command-specific nested graph records follow `src/types.ts` unless a section, such as experimental structured `query`, `relationships derive`, or `wiki`, is explicitly marked otherwise. |
...
| Dynamic Knowledge Card projection | experimental / stable-additive | `src/query/knowledge-card.ts`, `src/query/context-builder.ts`, `src/bin/mdgraph.ts`, `src/mcp/tools.ts` | Found document, section, entity, and source-ref `node` results may add a deterministic non-persisted `card`; selected context items may add `cardSummary`, counted in `usedChars`, only from budget left after original content. Existing node fields, ranking, trace paths, SQLite counts, and the five-tool MCP surface remain unchanged. |
