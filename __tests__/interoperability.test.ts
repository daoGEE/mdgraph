...
import { buildDocsSiteIndex, formatWikiLinkMarkdownIndex } from "../src/export/markdown-index.js";
import { buildSourceBridgeReport } from "../src/export/source-bridge.js";
...
describe("v0.7 interoperability exports", () => {
  ...
  it("verifies GraphJSON and reports actionable validation failures", async () => {
    ...
    try {
      ...
      const wrongFormat = { ...graph, format: "wrong-format" };
      const wrongFormatResult = verifyGraphJsonExport(wrongFormat);
      expect(wrongFormatResult.valid).toBe(false);
      expect(wrongFormatResult.errors.some((error) => error.code === "graphjson.format")).toBe(true);

      const missingGraphHash = { ...graph };
      delete (missingGraphHash as Partial<typeof graph>).graphHash;
      const missingGraphHashResult = verifyGraphJsonExport(missingGraphHash);
      expect(missingGraphHashResult.valid).toBe(false);
      expect(missingGraphHashResult.errors.some((error) => error.code === "graphjson.graph_hash")).toBe(true);
      ...
      const badHash = { ...graph, graphHash: "tampered-hash" };
      const badHashResult = verifyGraphJsonExport(badHash);
      expect(badHashResult.valid).toBe(false);
      expect(badHashResult.errors.some((error) => error.code === "graphjson.graph_hash")).toBe(true);
    } finally {
      repository.close();
    }
  });
  ...
  it("exports Markdown and docs-site indexes from GraphJSON without absolute paths", async () => {
    ...
    try {
      ...
      const markdown = formatWikiLinkMarkdownIndex(graph);
  ...
  it("builds a read-only JSON source bridge report without changing graph counts", async () => {
    const root = makeTempRoot("mdgraph-source-bridge-");
    createFixtureDocs(root);
    await indexProject(root);
    const artifactPath = path.join(root, "source-bridge.json");
    ...
    try {
      ...
      const unsupported = buildSourceBridgeReport(repository);
      expect(unsupported.status).toBe("unsupported");
      expect(unsupported.reason).toContain("No source bridge artifact");

      const invalidArtifact = path.join(root, "invalid-source-bridge.json");
      fs.writeFileSync(invalidArtifact, "{", "utf8");
      const invalid = buildSourceBridgeReport(repository, { artifact: invalidArtifact });
      expect(invalid.status).toBe("unsupported");
      expect(invalid.reason).toContain("could not be parsed");

      const deepArtifactPath = path.join(root, "deep-source-bridge.json");
      fs.writeFileSync(deepArtifactPath, nestedJson(130), "utf8");
      const deep = buildSourceBridgeReport(repository, { artifact: deepArtifactPath });
      expect(deep.status).toBe("unsupported");
      expect(deep.reason).toContain("JSON depth");

      const report = buildSourceBridgeReport(repository, { artifact: artifactPath });
