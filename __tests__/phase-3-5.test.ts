...
describe("phase 5 doctor", () => {
  ...
  it("reports definition collisions, orphan docs, weak links, and content risks", async () => {
    ...
    expect(report.health.storage.pathGroups.map((group) => group.group)).not.toContain("dist");
    expect(report.health.storage.warnings.map((warning) => warning.code)).not.toContain("storage.generated_path_indexed");
  });
