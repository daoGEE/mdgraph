...
describe("ToolHandler", () => {
  ...
  it("prefixes query results with a freshness warning when the index is stale", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mdgraph-mcp-query-stale-"));
    tempDirs.push(root);
    createFixtureDocs(root);
    await indexProject(root);
    ...
    const handler = new ToolHandler(root);
    const result = handler.execute("mdgraph_search", { query: "AuthService" });
    const content = result.structuredContent as SearchToolStructuredContent;

    expect(result.content[0].text).toContain("⚠️ MDGraph freshness warning");
    expect(result.content[0].text).toContain("serve --mcp");
    expect(content.freshness).toMatchObject({ state: "stale" });
    expect(content.freshness?.issues?.some((issue) => issue.path === "docs/auth-v2-design.md" && issue.reason === "modified")).toBe(true);
  });

  it("degrades mdgraph_status to freshness unknown when config loading fails", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "mdgraph-mcp-status-config-invalid-"));
    tempDirs.push(root);
    createFixtureDocs(root);
    await indexProject(root);
    ...
    expect(result.content[0].text).toContain("Freshness: unknown");
    expect(content.freshness).toMatchObject({ state: "unknown" });
    expect(content.freshness?.recommendation).toContain("freshness check failed while loading .mdgraph/config.json");
  });
